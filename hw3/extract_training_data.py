...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(state.stack)>=3:
            parser_state = [state.stack[-1], state.stack[-2], state.stack[-3]]
        elif len(state.stack)==2:
            parser_state = [state.stack[-1], state.stack[-2], None]
        elif len(state.stack)==1:
            parser_state = [state.stack[-1], None, None]
        elif len(state.stack)==0:
            parser_state = [None, None, None]
        
        if len(state.buffer)>=3:
            parser_state = parser_state + [state.buffer[-1], state.buffer[-2], state.buffer[-3]]
        elif len(state.buffer)==2:
            parser_state = parser_state + [state.buffer[-1], state.buffer[-2], None]
        elif len(state.buffer)==1:
            parser_state = parser_state + [state.buffer[-1], None, None]
        elif len(state.buffer)==0:
            parser_state = parser_state + [None, None, None]

        for i in parser_state:
            if (i is None) or (pos[i]=='<NULL>'):                  # <NULL>
                word_ind_list.append(4)
            elif (pos[i] == 'CD') or (pos[i] == 'NNP'):            # <CD> or <NP>
                word_ind_list.append(self.word_vocab["<{}>".format(pos[i])])
            elif (i==0) or (pos[i]=='<ROOT>'):                     # <ROOT>
                word_ind_list.append(self.word_vocab["<ROOT>"])
            else:
                word_index = self.word_vocab.get(words[i].lower())
                if (word_index is None) or (pos[i]=='<UNK>'):      # <UNK>
                    word_ind_list.append(self.word_vocab["<UNK>"])
                else: 
                    word_ind_list.append(word_index)
            
            input_representation = np.array(word_ind_list)  # re-format to numpy array
    ...
    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        labels=[]
        for i in self.output_labels.keys():
            labels.append(i)
            
        onehot_vector = np.zeros(91)
        for i in range(len(labels)):
            if output_pair == labels[i]:
                onehot_vector[i] = 1
